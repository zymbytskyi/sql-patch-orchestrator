...
function Get-BackupWarnings {
    ...
    foreach($backup in @($Backups)){
        ...
        try{$age=if($backup.PSObject.Properties.Name-contains'AgeDays' -and $null-ne$backup.AgeDays){[double]$backup.AgeDays}else{([datetime]::Now-[datetime]::ParseExact([string]$backup.LastFull,'yyyy-MM-dd HH:mm:ss',[Globalization.CultureInfo]::InvariantCulture)).TotalDays};if($age-gt$BackupWarningDays){$warnings.Add("$($backup.Database): last full backup is $([math]::Floor($age)) day(s) old")}}
...
function Write-Dashboard {
    ...
    $rows = foreach ($server in @($State.Servers)) {
        ...
        foreach($instance in $instances){
            ...
            $systemBackups=@($instance.Backups|Where-Object{$_.Database-in@('master','model','msdb')}|ForEach-Object{$kind=if((Get-PropertyValue $_ 'IsCopyOnly' $false)){'COPY_ONLY'}else{'FULL'};"<b>$((Encode-Html $_.Database))</b>: $((Encode-Html $_.LastFull)) <span class=`"muted`">$kind</span>"})-join'<br>'
...
SELECT d.name DatabaseName,d.state_desc State,b.backup_finish_date LastFullBackup,b.is_copy_only IsCopyOnly,b.has_backup_checksums HasChecksum,b.physical_device_name BackupPath,CASE WHEN b.backup_finish_date IS NULL THEN NULL ELSE DATEDIFF(MINUTE,b.backup_finish_date,GETDATE())/1440.0 END AgeDays FROM sys.databases d OUTER APPLY(SELECT TOP(1) bs.backup_finish_date,bs.is_copy_only,bs.has_backup_checksums,bmf.physical_device_name FROM msdb.dbo.backupset bs LEFT JOIN msdb.dbo.backupmediafamily bmf ON bmf.media_set_id=bs.media_set_id WHERE bs.database_name=d.name AND bs.type='D' ORDER BY bs.backup_finish_date DESC)b WHERE d.database_id<>2 ORDER BY d.database_id;
"@;$a=New-Object Data.SqlClient.SqlDataAdapter $q;$t=New-Object Data.DataTable;[void]$a.Fill($t);$info|Add-Member NoteProperty Backups @($t|ForEach-Object{[pscustomobject]@{Database=[string]$_.DatabaseName;State=[string]$_.State;LastFull=if($_.LastFullBackup-is[DBNull]){'NEVER'}else{([datetime]$_.LastFullBackup).ToString('yyyy-MM-dd HH:mm:ss')};IsCopyOnly=if($_.IsCopyOnly-is[DBNull]){$false}else{[bool]$_.IsCopyOnly};HasChecksum=if($_.HasChecksum-is[DBNull]){$false}else{[bool]$_.HasChecksum};BackupPath=if($_.BackupPath-is[DBNull]){''}else{[string]$_.BackupPath};AgeDays=if($_.AgeDays-is[DBNull]){$null}else{[double]$_.AgeDays}}});$instances+=$info}finally{if($c){$c.Dispose()}}
