...
$p=Join-Path $env:TEMP 'Install-SqlPatchOrchestrator-3.0.0.ps1'
Invoke-WebRequest 'https://raw.githubusercontent.com/zymbytskyi/sql-patch-orchestrator/v3.0.0/Install-FromGitHub.ps1' -UseBasicParsing -OutFile $p
...
powershell.exe -NoProfile -ExecutionPolicy Bypass -File $p -Version 3.0.0
```
...
Download `SqlPatchOrchestrator-v3.0.0.zip` from [Releases](https://github.com/zymbytskyi/sql-patch-orchestrator/releases/tag/v3.0.0), extract it to `C:\`, then run `C:\SqlPatchOrchestrator\Install.cmd` as administrator. The archive already contains the correct top-level folder.
...
2. Inventory, review backup dates, and optionally create verified COPY_ONLY backups of system databases.
