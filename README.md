...
$p=Join-Path $env:TEMP 'Install-SqlPatchOrchestrator-3.0.5.ps1'
Invoke-WebRequest 'https://raw.githubusercontent.com/zymbytskyi/sql-patch-orchestrator/v3.0.5/Install-FromGitHub.ps1' -UseBasicParsing -OutFile $p
...
powershell.exe -NoProfile -ExecutionPolicy Bypass -File $p -Version 3.0.5
```

The installer verifies the GitHub release SHA-256 digest and installs to `C:\SqlPatchOrchestrator`. V3.0.5 does not start a blocking Microsoft Defender custom scan. It does not disable Defender, create exclusions, or change endpoint security policy. Existing targets, packages, and runs are preserved during a non-interactive upgrade.

## Install from ZIP

Download `SqlPatchOrchestrator-v3.0.5.zip` from [Releases](https://github.com/zymbytskyi/sql-patch-orchestrator/releases/tag/v3.0.5), extract it to `C:\`, then run `C:\SqlPatchOrchestrator\Install.cmd` as administrator. The archive already contains the correct top-level folder.
