<# .SYNOPSIS Tests the V3.0.6 cycle, credential retry, inventory, download, installer, and system-backup contracts. #>
...
if($version-ne'3.0.6'-or$readme-notmatch'v3\.0\.6'){$failures.Add('Version, README, and release tag are inconsistent.')}
$credentialContext=Select-String -InputObject $menu -Pattern '\$common\.Credential=\$Credential' -AllMatches
if($credentialContext.Matches.Count-lt2){$failures.Add('Alternate credential is not retained in the common phase arguments after retry.')}
if($menu-notmatch[regex]::Escape('$common.Remove(''Credential'')')-or$menu-notmatch[regex]::Escape('$Credential=$null')){$failures.Add('Menu does not clear its alternate credential reference when it exits.')}
...
$testRoot=Join-Path ([IO.Path]::GetTempPath()) ('SqlPatchV306-'+[guid]::NewGuid().ToString('N'))
try{
...
if($Mode-eq'Inventory'-and-not$Credential){$state=[pscustomobject]@{Cycle=$Cycle;Stage='InventoryBlocked';Servers=@([pscustomobject]@{Server='VMAP072';Status='Failed';Message='Access is denied.';Instances=@()})};$global:LASTEXITCODE=1}
...
'@
    ...
    $global:SqlPatchTestMenuAnswers.Clear();foreach($answer in @('Credential retry test','2','y','0','','0')){$global:SqlPatchTestMenuAnswers.Enqueue($answer)}
    function global:Get-Credential{param([string]$Message);$secure=New-Object Security.SecureString;return (New-Object Management.Automation.PSCredential('CONTOSO\TestDba',$secure))}
    $credentialOutput=(& (Join-Path $mockRoot 'Start-SqlPatchV3Menu.ps1') -RunRoot (Join-Path $mockRoot 'Runs') 6>&1|Out-String)
    if($credentialOutput-notmatch[regex]::Escape("WinRM reached VMAP072, but Windows rejected account")-or$credentialOutput-notmatch[regex]::Escape("Retrying Inventory as 'CONTOSO\TestDba'")-or$credentialOutput-notmatch'Retry Inventory exit code: 0'){$failures.Add('Access-denied Inventory did not retry successfully with the in-memory alternate credential.')}
...
}finally{Remove-Item Function:\global:Read-Host -ErrorAction SilentlyContinue;Remove-Item Function:\global:Get-Credential -ErrorAction SilentlyContinue;Remove-Variable SqlPatchTestMenuAnswers -Scope Global -ErrorAction SilentlyContinue;if(Test-Path $testRoot){Remove-Item -LiteralPath $testRoot -Recurse -Force}}
...
Write-Output '[PASS] Access-denied alternate credential retry and in-memory reuse contract.'
