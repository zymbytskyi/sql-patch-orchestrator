<# .SYNOPSIS Tests the V3.0.4 cycle, inventory, download, installer, and system-backup contracts. #>
...
foreach($required in @('Select-PatchCycle','ConvertTo-PatchCycle','Use YYYY-MM','GetCultures','8. Select or create another patch cycle','Show-InventoryBlockers',"Stage-notin@('InventoryReady','Prepared','PreflightReady')")){if($menu-notmatch[regex]::Escape($required)){$failures.Add("Menu behavior missing: $required")}}
...
if($version-ne'3.0.4'-or$readme-notmatch'v3\.0\.4'){$failures.Add('Version, README, and release tag are inconsistent.')}
...
$testRoot=Join-Path ([IO.Path]::GetTempPath()) ('SqlPatchV304-'+[guid]::NewGuid().ToString('N'))
try{
    New-Item -ItemType Directory -Path $testRoot -Force|Out-Null
    $global:SqlPatchTestMenuAnswers=New-Object Collections.Generic.Queue[string]
    foreach($answer in @('2026-09','3','','0')){$global:SqlPatchTestMenuAnswers.Enqueue($answer)}
    ...
    if($localizedOutput-notmatch'Cycle: August2026'){$failures.Add('Localized full month name did not normalize to August2026.')}
    try{& (Join-Path $root 'Invoke-SqlPatchV3Remote.ps1') -Mode Dashboard -Cycle '..\bad' 2>$null;$failures.Add('Unsafe cycle path was accepted.')}catch{if($_.Exception.Message-notmatch'Patch cycle is invalid'){$failures.Add('Unsafe cycle path returned the wrong validation message.')}}
...
Write-Output '[PASS] Language-neutral and localized cycle input normalization.'
